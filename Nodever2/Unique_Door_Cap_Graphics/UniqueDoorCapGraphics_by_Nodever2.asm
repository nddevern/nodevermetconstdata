...
;=======================DEFINES=======================
{
	...
	;=====PRIMARY TWEAKS=====
	{
		...
		;---SFX VALUES---
		{
			...
			!OpenSound   = $2C ; vanilla $07. The SFX that plays when opening a door.
			!ClosSound   = $15 ; vanilla $08. The SFX that plays when a door is closing.
		...
		;---DOOR HEALTH VALUES---
		{
			; These can all be safely changed. Valid values are from 01-1F.
			...
			!BlueDoorsReactToPBs = 0 ; SET THIS TO 0 IF YOU WANT POWER BOMBS TO OPEN BLUE/GREY DOORS. SET TO -2 OTHERWISE.
	...
	;=====DEFINES YOU SHOULDN'T TOUCH UNLESS YOU KNOW WHAT YOU'RE DOING=====
	{
        ...
        ; THESE ARE ALSO USED AS AN INDEX INTO DMA SOURCE ADDRESS TABLES.
		...
		;---PLM RAM values---
		{
			...
			{
				; For new doors, at first contains grey door type, respawn flag, DMA graphics index, and door ID (See "RELEVANT DATA STRUCTURES" section). 
			...
			!RAMPLMVars2            = $7EDF0C ; $7EDF0C..5B: PLM variables
			{
                ...
                ; Low byte:
				;   Highest 3 bits are palette bits used by door DMA inst
                ;   Lowest 5 bits are remaining door HP
...
;===============RELEVANT DATA STRUCTURES==============
{
	...
	{
        ; TODO ADJUST GREY DOOR PRE INSTRUCTION CODE TO READ PROPERLY FROM THIS NOW SINCE I MOVED THAT. ALSO ADJUST FLIPS TO NOT BE SET BASED ON ROOM ARG.
		; rgggpppddddddddd
		;  r: Door cap will respawn if this bit is set
        ;  g: Index of room graphics slot to DMA graphics into when applicable. 0-7. In my hack loads into CRE tiles 2E0-2FF.
		;  p: Index of grey door pre instruction to use. Should be zero for all non-grey doors. vanilla options are 0-6.
...
macro ColoredDoorSetup(CLR)
{
	...
	Door<CLR>InvertedVSetup:
        LDA #$0010 : JSR SetDoorTile_InvertedV      ; set V flipped door tile
        BRA ?finishclrVsetup
        
	Door<CLR>NormalVSetup:
		LDA #$0010 : JSR SetDoorTile_V              ; set door tile	
...
macro ColoredDoorInstList(O)
{
	...
	DoorCLR<O>Close:
        ...
        dw InstDMADoor<O> : db !DMAf1, !EndAnimSpIndex ; instant. also TODO make these blue palette???
		dw InstDrawDoor<O> : db !DoorCLR<O>Frame4Offset, !TTshotbyte|!ClosSpeedIndex ; commit DMA graphics
		dw InstDMADoor<O> : db !DMAf2, !ClosSpeedIndex
        dw InstDMADoor<O> : db !DMAf3, !ClosSpeedIndex
	...
	DoorCLR<O>Hit:
		...
		dw InstDMADoor<O> : db !DMAfw, !HitBSpeedIndex ;dw InstDrawBLUDoor<O> : db !DoorBLU<O>Frame4Offset, !TTshotbyte|!HitBSpeedIndex
		dw InstDMADoor<O> : db !DMAf4, !HitCSpeedIndex ;dw InstDrawDoor<O>    : db !DoorCLR<O>Frame4Offset, !TTshotbyte|!HitCSpeedIndex
		dw InstDMADoor<O> : db !DMAfw, !HitBSpeedIndex ;dw InstDrawBLUDoor<O> : db !DoorBLU<O>Frame4Offset, !TTshotbyte|!HitBSpeedIndex
		dw InstDMADoor<O> : db !DMAf4, !HitCSpeedIndex ;dw InstDrawDoor<O>    : db !DoorCLR<O>Frame4Offset, !TTshotbyte|!HitCSpeedIndex
		dw InstDMADoor<O> : db !DMAfw, !HitBSpeedIndex ;dw InstDrawBLUDoor<O> : db !DoorBLU<O>Frame4Offset, !TTshotbyte|!HitBSpeedIndex
		dw InstDMADoor<O> : db !DMAf4, !HitCSpeedIndex ;dw InstDrawDoor<O>    : db !DoorCLR<O>Frame4Offset, !TTshotbyte|!HitCSpeedIndex
	...
	?open:
		...
		dw InstDMADoor<O> : db !DMAf1, !OpenSpeedIndex ;dw InstDrawNoCapDoor<O> : db $00,                     !TTairbyte|!EndAnimSpIndex
		dw InstDrawNoCapDoor<O> : db $00, !TTairbyte|!EndAnimSpIndex
        dw !InstDelete
        
        ;NOTE TO SELF (TODO DELETE): ANY TIME YOU CHANGE THE PALETTE , YOU SHOULD... well hmm... MAYBE YOU NEED A DRAW INSTRUCTION? NOT SURE.
        ; BUT FOR REAL, ANYTIME YOU CHANGE TILE TYPE, YOU MUST USE A DRAW INSTRUCTION.
...
macro GreyDoorSetup()
{
	DoorGRYInvertedHSetup:
        LDA #!GRYHDoorTile : JSR SetDoorTile_InvertedH         ; set H flipped door tile
		BRA ?finishgryHsetup
		
	DoorGRYNormalHSetup:
		LDA #!GRYHDoorTile : JSR SetDoorTile_H                 ; set door tile
	...
	DoorGRYInvertedVSetup:
		LDA #!GRYVDoorTile : JSR SetDoorTile_InvertedV         ; set V flipped door tile
        BRA ?finishgryVsetup
		
	DoorGRYNormalVSetup:
		LDA #!GRYVDoorTile : JSR SetDoorTile_V                 ; set door tile
...
macro GreyDoorInstList(O)
{
	...
	?open:
		dw !InstOpenSFX : db !OpenSound
		dw InstDrawWHTDoor<O>   : db $00,                     !TTairbyte|!OpenSpeedIndex
		dw InstDrawBLUDoor<O>   : db !DoorGRY<O>Frame2Offset, !TTairbyte|!OpenSpeedIndex
		dw InstDrawBLUDoor<O>   : db !DoorGRY<O>Frame1Offset, !TTairbyte|!OpenSpeedIndex
		dw InstDrawNoCapDoor<O> : db $00,                     !TTairbyte|!EndAnimSpIndex
		dw !InstDelete
}
endmacro
...
macro BlueDoorInstList(O)
{
	...
	DoorBLU<O>Main:
		dw !InstOpenSFX : db !OpenSound
		dw InstDrawWHTDoor<O>   : db $00,                     !TTairbyte|!OpenSpeedIndex ; we could use InstDrawBLUDoor but in this case it's the same with slightly higher execution time
...
macro EyeDoorSetup()
{
	...
	DoorEYERSetupEye:
		LDA #!BLUHDoorTile : ORA #$0400 : STA !RAMPLMVars,x ; set H flipped door tile
        BRA ?finisheyeHsetup
...
;================PLM DRAW INSTRUCTIONS================
{
    ...
    ; Instruction: InstDMADoor. DMAs door tiles for a given animation frame of a door into the door's respective CRE dynamic door gfx index (ggg in the room arg). Note: If animating a door using DMA, we will always use frame4offset in InstDrawDoor argument.
    ; Arguments:
    ; [Y]: anim frame to DMA, can do f4-f1 and white door animframe. 1 byte.
    ; [Y+1]: Animation speed. Unlike InstDrawDoor, !EndAnimSpeed causes a delay of ZERO bytes instead of 1. This is for instant animation.
...

    {
        ...
        ; store from !RAMPLMVars2,x into $7EA000
        ; palette bits are the #$00E0 bits in !RAMPLMVars2,x.
		JSR SetHDoorDMAPalette
        ; also get color palette bits and store them in $7EA000
        ; format YXLP PPTT TTTT TTTT Flips, priority, palette, tile
        
        InstDMADoorV:
        ; get source address for V door graphics
        ; remember, high byte of !RAMPLMVars2,x is index into DoorSpeedsTables to use for this door color; we can use that same index into a table of door animation frame tables.
        ...
        InstDMADoor:
        ; reminder: y is index into bank for next PLM instruction/arg,
        ;           x is PLM index
        ...
        ; for $D0,x table:
        ;  Size is 2 bytes
        ;  Source addr is 3 bytes
        ;  Dest   addr is 2 bytes
        LDA #$0080 : STA $00D0,y ; size (should always be 4 tiles for noneye doors, and each tile is $20 bytes)
        LDA #$00DF : STA $00D4,y ; source bank (only low byte in A matters, the rest will be overwritten)
        PLY
        
        ; LOAD SOURCE ADDR
        PHX
        LDA !RAMPLMVars2,x : XBA : AND #$00FF : ASL : TAX
        LDA DoorDMAGfxAddrTables,x : PHA
    ...
    +   ASL
        ...
        ; load value from that table
        
        CLC : ADC $01,s : TAX
        
        LDA $840000,x ; load value from <CLR>DoorDMAGfxAddrs table
        
        PLX : PLX ; DONE LOADING SOURCE ADDR

        ;from my anim tiles guide:
...
;Mul    tiply this number (in the case of vertical spikes, $388) by $10 to get your VRAM address ($3880).

        
        PHY : LDY $0330
        STA $00D2,y ; source address
        ; for dest addr: multiply door DMA graphics slot index by $40 since each door takes 4 tiles and $10 per tile
        LDA !RAMPLMRoomArgs,x : AND #$7000 : XBA ; VRAM destination address
        ASL : ASL : CLC : ADC #$2E00 : STA $00D5,y ; todo not hardcode this VRAM address to 2E00

        TYA : CLC : ADC #$0007 : STA $0330
        PLY
        
        ;handle anim speed. Also increments Y.
        JSR LoadDoorAnimSpeed : BCC .returnwithdelay ; TODO - for some reason these always animate at 1 animation frame delay always. this is a bug,.
    .returnwithoutdelay
        RTS
    ...
    .returnwithdelay
        ...
        GRYDoorDMAGfxAddrs:
            ...
            dw GRYDoorDMAGfxf1  ; f1 - almost fully open
		...
		SetVDoorDMAPalette:
			PHX
			LDA !RAMPLMVars2,x : AND #$00E0 : ASL #5 : STA $02
			LDA !RAMPLMVars,x : AND #$03FF : ASL #3 : TAX ; get index of tile to modify
			LDA $7EA080,x : AND #$E3FF : ORA $02 : STA $7EA080,x
			LDA $7EA082,x : AND #$E3FF : ORA $02 : STA $7EA082,x
			LDA $7EA088,x : AND #$E3FF : ORA $02 : STA $7EA088,x
			LDA $7EA08A,x : AND #$E3FF : ORA $02 : STA $7EA08A,x
			PLX
			RTS
		...
		SetHDoorDMAPalette:
			...
			LDA !RAMPLMVars2,x : AND #$00E0 : ASL #5 : STA $02
			LDA !RAMPLMVars,x : AND #$03FF : ASL #3 : TAX ; get index of tile to modify
			LDA $7EA000,x : AND #$E3FF : ORA $02 : STA $7EA000,x
			LDA $7EA004,x : AND #$E3FF : ORA $02 : STA $7EA004,x
			LDA $7EA100,x : AND #$E3FF : ORA $02 : STA $7EA100,x
			LDA $7EA104,x : AND #$E3FF : ORA $02 : STA $7EA104,x
			PLX
			RTS
...
;====================PLM SETUPS======================
{
	...
	{
		LDA #$0000 : STA !RAMPLMTimers,x                            ; clear door shot status
...
;==============NEW PLM INSTRS+FUNCTIONS==============
{
	...
	InstDrawWHTDoorH:
	{
		LDA   #!WHTHDoorTile             ; Set new (temporary) base door tile to use
	...
	{
		SEP #$20
		LDA !RAMPLMVars2,x : AND #$1F ; remaining door HP
		DEC
        BEQ .doordead
        BIT #$E0 : BNE .doordead
        JSR CommitDoorHealth
		REP #$20
		INY : INY
	.rt	RTS
	
    .doordead
        LDA #$00 : JSR CommitDoorHealth ; door HP is zero
		REP #$20
        PHX
		LDA !RAMPLMRoomArgs,x
		BMI + ; if door respawn bit set, don't set PLM room argument door
        AND #$81FF
	...
	+	PLX
        LDA !RAMPLMRoomArgs,x ; FIX BUG WHICH EXISTS IN THE RELEASE VERSION OF MY PATCH...
		ORA #$8000
	...
	SetGreyDoorPreInst:
	{
		...
		XBA : AND #$000E : TAX       ;) Set grey door pre-inst from room arg
		LDA .preinsts,x				 ;|
		STA !RAMPLMPreInsts,y		 ;/
		
		LDA !RAMPLMRoomArgs,y		 ;\
		AND #$F1FF					 ;) mask out grey door pre inst index bits from PLM room arg (why do we need this again?)
	...
	{
		PHP : SEP #$20 : PHA
		LDA !RAMPLMVars2,x : AND #$1F
		SEC : SBC $01,s : BIT #$E0 : BNE .negativehealth
		.commithealth
        JSR CommitDoorHealth
		PLA : PLP : RTS
        .negativehealth
        LDA #$00 : BRA .commithealth
	}
    
    ; Function: Commit a new door health value in [A] to !RAMPLMVars2,x
    ; MUST SEP #$20 BEFORE CALLING THIS AND REP #$20 AFTER.
    CommitDoorHealth:
    {
        STA $02
        LDA !RAMPLMVars2,x : AND #$E0 : ORA $02
        STA !RAMPLMVars2,x
        RTS
    }
    ...
    .V
        JSR .getDMAGraphicsSlotIndex : LSR #5
        
    .finish
        CLC : ADC !RAMPLMVars,y        ;) add to door tile
        STA !RAMPLMVars,y              ;/
        RTS
    
    .getDMAGraphicsSlotIndex
        STA !RAMPLMVars,y
        LDA !RAMPLMRoomArgs,y : AND #$7000 : XBA
        RTS
    }
    
    ; Function: Get door speed (TODO/WIP)
	...
	DeletePLMAndAbortSetupIfRoomArg:
	{
		...
		AND #$81FF : JSL !FuncLBitIndexToByteIndexAndBitmask
