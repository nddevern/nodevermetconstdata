...
; =================================================
{
    ...
    !PlaceSamusAlgorithm        = 1  ; PlaceSamusAlgorithm: Determines which algorithm is used to place Samus after a door transition:
                                     ...
                                     ;     5: Samus is placed at the destination door cap position X AND Y. This does not just affect her distance from the edge of the screen. Takes effect regardless of if door cap is enabled. Ignores distance to spawn field.
                                     ...
                                     ;        8001-FFFD: Acts as algorithm 3, but ignores bit 8000h. All other bits are used as a hardcoded pixel offset from the edge of the screen.
                                     ;        FFFE: Acts as algorithm 2 but slightly modified. Samus is placed at destination door cap distance, even if there is no door cap enabled.
                                     ;        FFFF: Acts as algorithm 5.
    ...
    !RamCameraYTableIndex                        := !CurRamAddr : !CurRamAddr := !CurRamAddr+2 ; if negative, run the door transition as fast as possible and don't read from the table.
...
; ====================================
{
    ...
    ; todo: add support for non-fixed-length door transition animations...
    ; this would give full parity with vanilla.
    ; the transition length is not a set amount of time but is determined by how far the door has to align.
    ; so you just give it a px/frame value.
}
...
; =======================================================
; ==== SCREEN FADE TO/FROM BLACK + DISABLE ANIMATION ====
; =======================================================
{
    if !ScreenFadesOut == 0
    ...
    ; Returns: Carry set if door animation is needed, Carry clear otherwise.
    CheckIfDoorScrollingAnimationIsNeeded: {
            PHP : REP #$30 : PHX : PHA
            if !AddOptionToFadeLayer1 > 0 || !AddOptionToSkipScrolling > 0
                LDX $078D : LDA $830002,x ; A = [$83:0000 + [door pointer] + 2] (elevator properties)
            endif
            if !AddOptionToFadeLayer1 > 0
                BIT #$0020 : BNE .no
            endif
            if !AddOptionToSkipScrolling > 0
                BIT #$0010 : BNE .no
            endif
            LDX $079B : LDA $8F0008,x ; A = [$8F:0000 + [room header] + 2] (CRE bitset)
            BIT #$0001 : BNE .no      ; bit 01 = disable layer 1 (fade all colors)
        .yes
            PLA : PLX : PLP : SEC : RTL
        .no
            PLA : PLX : PLP : CLC : RTL
    ...
    ; Returns: Carry set if all colors need faded, carry clear otherwise.
    CheckIfColorsNeedFaded: {
        PHP : REP #$30 : PHX
        ORA $07B1 : BIT #$0001 : BNE .no ; instructions replaced by hijack (effectively)
        if !AddOptionToFadeLayer1 > 0
            LDX $078D : LDA $830002,x : BIT #$0020 : BEQ .no
        endif
        .yes
            PLX : PLP : SEC : RTL
        .no
    ...
    !FreespaceAnywhere := CheckIfColorsNeedFaded_freespace
    warnpc !FreespaceAnywhereEnd

    org $82E224
        NOP #2 : JSL CheckIfColorsNeedFaded : BCS $3F
}
...
; =======================================================
{
    ...
    MainScrollingRoutine: {
            ...
            JSL CheckIfDoorScrollingAnimationIsNeeded : BCS +++
            ; flag CameraTableIndexes as negative to indicate that we need to scroll the screen as fast as possible
            LDA !RamCameraXTableIndex : ORA #$8000 : STA !RamCameraXTableIndex
            LDA !RamCameraYTableIndex : ORA #$8000 : STA !RamCameraYTableIndex
            +++
    ...
    ScrollCamera: {
        ...
        !tLayer2StartPos                          = !tStackOffset+16,s
            ...
            LDA !tCameraTableIndex : BMI .fastCamera
        ...
        .gotLookupTableValue
            ...
            TYA : CMP #$0100 : BEQ .setCameraPosition
            BRA .divideLookupTableValue ; convert to a %
        .setCameraPosition
        ...
        .fastCamera
            ; convert tCameraTableIndex into a value from #$0000 to #$0100 - move the camera 10hpx/frame
            AND #$7FFF : BNE + 
            ; special case: only move 1 px on the first frame
            INC : BRA ++
        +   ASL #4           ; multiply by 10h to convert counter into pixel offset (01h -> 0010h)
            CMP #$0100 : BPL .finish
        ++  PHK : PLB ; DB = current bank
            BRA .gotLookupTableValue

        .divideLookupTableValue
...
; ======================================================================
{
    ...
    PositionSamus: {
        ...
        .mine
            ...
            if !PlaceSamusAlgorithm == 4
                BRA ..doorcap ; use destination it's X and Y positions to set Samus - regardless of door cap
            endif
        ...
        if !PlaceSamusAlgorithm == 5 || !PlaceSamusAlgorithm == 4
        .directDoorPosition
            JSR .mySetSamusScreenX
            JSR .mySetSamusScreenY
            LDX $078D : LDA $830004,x : TAX ; X = [$83:0000 + [door pointer] + 4] (X and Y positions)
...
; ===============================================
{
    ...
    org $808F6D
        NOP : JSL AsyncSpcUpload_InitializeUpload
    ...
    AsyncSpcUpload: {
        ...
        .InitializeUpload:
            ; X = music data index
            STA $064C : REP #$20 ; Instructions replaced by hijack
            PHA
